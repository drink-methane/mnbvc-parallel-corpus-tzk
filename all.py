...
LANGUAGES = { # 将需要按语言写死的部分，全部放到这一个表中，如果有组合数据，应该考虑用dict
    'Chinese':('简体中文','zh_text'), # 如果需要语言重命名，或者打印语言信息，可以像这样把每个语言的设置拆成 tuple 来写。这个表的意义是建立语言的映射关系
    # 'en':('英文','en_text'),
    'French':('法语','fr_text'),
    'German':('德语','de_text'),
    # 'it':('意大利语','it_text'),
    ...
    # 'epo':('世界语','eo_text'),
    # 'nl':('荷兰语','nl_text'),
    'Traditional Chinese':('繁体中文','cht_text'),
    # 'vie':('越南语','vi_text'),
    # 'ukr':('乌克兰语','uk'), # 填扩展字段里
    ...
    # 'pt-BR':('巴西葡语','pt_BR'),# 填扩展字段里
    # 'sv':('瑞典语','sv'),# 填扩展字段里
    # 'da':('丹麦语','da')# 填扩展字段里
}
mate_dir = Path(r"D:\sandbox\data")
filename = "discoElysium.jsonl"
max_lines = 500000
max_size = 512
