...
directory = r"D:\MNBVC\HollowKnight\HollowKnight"
pattern = r'<[^>]*>'
...
for root, dirs, files in os.walk(directory):
    for file in files:
        ...
        for entry in xmlroot.findall('entry'):
            name = entry.get('name')#re.sub(pattern, '', str(entry.text).strip())
            text = re.sub(pattern, '', str(entry.text).strip())
